...
def _ioc_full_worker(task):
    """Score every full decode by IoC over the FULL 26^4 ring space. The step sequence
    depends on RM iff the MIDDLE rotor has a core-notch (its double-step is offset-based,
    e.g. III in the middle), otherwise on RR (the right rotor's turnover). We loop over
    the ring that drives the stepping and vectorise the other three (same 26^4 total)."""
    import numpy as np
    ...
    mid_off = cs.NOTCH_OFFSET.get(order[1]) is not None
    RU, RL, Ro = (z.ravel() for z in np.meshgrid(g, g, g, indexing='ij'))          # 26^3
    V = RU.size
    res = []
    for Lv in range(26):                       # Lv is RM if mid_off else RR
        if mid_off:
            seq = cs.posseq(order, windows, N, (0, Lv, 0)); RMc = Lv; RRc = Ro
        else:
            seq = cs.posseq(order, windows, N, (0, 0, Lv)); RRc = Lv; RMc = Ro
        out = np.empty((V, N), np.int8)
        for t in range(N):
            L, M, R = seq[t]
            oL = (L - RL) % 26; oM = (M - RMc) % 26; oR = (R - RRc) % 26; u = (uwin - RU) % 26
            x = ETWR[int(ct[t])]
        ...
        for i in idx:
            ...
            rmv = Lv if mid_off else int(Ro[i])
            rrv = int(Ro[i]) if mid_off else Lv
